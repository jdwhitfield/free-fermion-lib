...
def stabilizer_distribution(rho):
    ...
    for j, Pj in enumerate(pauli_group):
        ...
        if abs(expectP.imag) < 1e-9:
            ...
            raise ValueError(
                f"Significant imaginary part in expectation value: {expectP}"
            )
    ...
    assert np.allclose(
        np.sum(xi), 1
    ), f"Distribution not normalized: sum = {np.sum(xi)}"
...
def cov_distribution(rho):
    ...
    assert np.allclose(
        np.linalg.norm(M + M.T), 0
    ), "Covariance matrix is not antisymmetric"
