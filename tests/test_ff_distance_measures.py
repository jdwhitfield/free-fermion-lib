...
class TestStabilizerDistribution:
    ...
    def test_wavefunction_vs_density_matrix(self):
        ...
        assert np.allclose(
            xi_psi, xi_rho
        ), "Wavefunction and density matrix should give same result"
...
class TestStabilizerRenyiEntropy:
    ...
    def test_sre_bounds(self):
        ...
        assert sre_mixed >= -1e-10, "SRE should be non-negative, instead: " + repr(
            sre_mixed
        )
    ...
    def test_different_renyi_parameters(self):
        ...
        assert np.isreal(sre_1) and np.isfinite(
            sre_1
        ), "SRE(α=1) should be real and finite"
        assert np.isreal(sre_2) and np.isfinite(
            sre_2
        ), "SRE(α=2) should be real and finite"
        assert np.isreal(sre_3) and np.isfinite(
            sre_3
        ), "SRE(α=3) should be real and finite"
    ...
    def test_wavefunction_input(self):
        ...
        assert np.isreal(sre_psi) and np.isfinite(
            sre_psi
        ), "Should handle wavefunction input"
...
class TestLinearEntropy:
    ...
    def test_bounds(self):
        ...
        for p in distributions:
            ...
            assert (
                0 <= lin_ent <= (len(p) - 1) / len(p) + 1e-10
            ), "Linear entropy should be in valid range"
    ...
    def test_relationship_to_purity(self):
        ...
        assert np.allclose(
            lin_ent, expected_lin_ent
        ), "Should match purity relationship"
...
class TestFermionicAntiFlatness:
    ...
    def test_different_k_parameters(self):
        ...
        # All should be real and finite
        assert np.isreal(faf_1) and np.isfinite(
            faf_1
        ), "FAF(k=1) should be real and finite"
        assert np.isreal(faf_2) and np.isfinite(
            faf_2
        ), "FAF(k=2) should be real and finite"
        assert np.isreal(faf_3) and np.isfinite(
            faf_3
        ), "FAF(k=3) should be real and finite"
    ...
    def test_wavefunction_input(self):
        ...
        assert np.isreal(faf_val) and np.isfinite(
            faf_val
        ), "Should handle wavefunction input"
    ...
    def test_column_vector_input(self):
        ...
        assert np.isreal(faf_val) and np.isfinite(
            faf_val
        ), "Should handle column vector input"
...
class TestInputValidation:
    ...
    def test_non_hermitian_density_matrix(self):
        ...
        try:
            ...
            assert isinstance(
                result, np.ndarray
            ), "Should return array even for non-Hermitian input"
        except Exception:
    ...
    def test_negative_probabilities(self):
        ...
        try:
            ...
            assert np.isfinite(s) or np.isfinite(
                s.real
...
class TestSpecificKnownCases:
    ...
    def test_bell_states(self):
        ...
        assert np.allclose(
            np.sum(xi), 1.0
        ), "Bell state should give normalized distribution"
    ...
    def test_ghz_states(self):
        ...
        assert np.allclose(
            np.sum(xi), 1.0
        ), "GHZ state should give normalized distribution"
    ...
    def test_product_states(self):
        ...
        assert np.allclose(
            np.sum(xi), 1.0
        ), "Product state should give normalized distribution"
...
class TestCrossValidation:
    ...
    def test_renyi_entropy_consistency(self):
        ...
        assert np.allclose(
            s1_renyi, s1_shannon, rtol=1e-10
...
class TestDocumentationExamples:
    ...
    def test_renyi_entropy_examples(self):
        ...
        assert (
            s2 <= s1
        ), "Collision entropy should be less than or equal to Shannon entropy"
