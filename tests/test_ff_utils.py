...
class TestPartialTraceFunctions:
    ...
    def test_which_is_which(self):
        ...
        assert np.allclose(
            rho1, A * np.trace(B)
        ), "partial_trace_over_2 should trace out subsystem 2"
        assert np.allclose(
            rho2, B * np.trace(A)
        ), "partial_trace_over_1 should trace out subsystem 1"
    ...
    def test_partial_trace_bell_states(self):
        ...
        assert np.allclose(
            rho_A2, expected_mixed
        ), "All Bell states give maximally mixed reduced states"
        assert np.allclose(
            rho_B2, expected_mixed
        ), "All Bell states give maximally mixed reduced states"
    ...
    def test_partial_trace_consistency(self):
        ...
        assert np.allclose(
            np.trace(rho_A), np.trace(rho_B)
        ), "Both partial traces should have same trace"
        ...
        assert np.allclose(
            rho_A_sym, rho_B_sym
        ), "Symmetric system should give identical reduced states"
    ...
    def test_partial_trace_known_analytical_results(self):
        ...
        assert np.allclose(
            rho_A, expected, atol=1e-10
        ), "Werner state should give maximally mixed reduced states"
        assert np.allclose(
            rho_B, expected, atol=1e-10
        ), "Werner state should give maximally mixed reduced states"
        ...
        assert np.allclose(
            rho_A_23, expected_A
        ), "Known 2×3 example should match analytical result"
        assert np.allclose(
            rho_B_23, expected_B
        ), "Known 2×3 example should match analytical result"
    ...
    def test_partial_trace_properties_verification(self):
        ...
        for _ in range(5):
            ...
            assert np.allclose(
                np.trace(rho_A), 1.0, atol=1e-12
            ), "Trace should be preserved"
            assert np.allclose(
                np.trace(rho_B), 1.0, atol=1e-12
            ), "Trace should be preserved"

            # Property 2: Hermiticity
            assert np.allclose(
                rho_A, rho_A.conj().T, atol=1e-12
            ), "Reduced state should be Hermitian"
            assert np.allclose(
                rho_B, rho_B.conj().T, atol=1e-12
            ), "Reduced state should be Hermitian"
            ...
            assert rho_A.shape == (
                2,
                2,
            ), "Reduced state A should have correct dimensions"
            assert rho_B.shape == (
                2,
                2,
            ), "Reduced state B should have correct dimensions"
    ...
    def test_partial_trace_linearity(self):
        ...
        assert np.allclose(
            rho_combined_A, expected_A, atol=1e-12
        ), "Partial trace should be linear"
        assert np.allclose(
            rho_combined_B, expected_B, atol=1e-12
        ), "Partial trace should be linear"
